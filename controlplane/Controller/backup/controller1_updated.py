...
DP_THRESHOLD = 1000
# ================= Ground Truth Config =================
GT_VICTIM = "9.9.9.9"   # must match attack_gen.py
# =======================================================
...
class myController(object):
    def __init__(self):
        ...
        if not os.path.exists(self.out_csv):
            with open(self.out_csv, "w", newline="") as f:
                w = csv.writer(f)
                w.writerow(["epoch_ts", "victim_ip", "cm_count", "lemon_est"])
        self.prev_lemon = {}
    ...
    def query(self):
        ...
        for hh_id in range(0, self.heavysize):
            ...
            logging.info(f"DETECT {ip},{c},{Est}")
            with open(self.out_csv, "a", newline="") as f:
                w = csv.writer(f)
                w.writerow([epoch_ts, ip, int(c), float(Est)])
            ...
            label = get_true_label(ip, epoch_ts)
            # ---------------- DP decision ----------------
            ...
            # ---------------- Confusion ----------------
            dp_result = confusion(dp_pred, label)
            cp_result = confusion(cp_pred, label)
            ...
            logging.info(f"[CVFDT] pred={y_pred}, label={label}, features={features}")
            logging.info(f"[STATS] DP?CP events so far: {self.dp_to_cp_events}")
 
            # ================= CVFDT INTEGRATION END =================

        # --- epoch-level summary + confusion matrices (includes 'no alert')
        epoch_label = get_epoch_label(epoch_ts, saw_gt_victim)
        ...
        logging.info(f"[CM_DP] TP={self.cm_dp['TP']} TN={self.cm_dp['TN']} FP={self.cm_dp['FP']} FN={self.cm_dp['FN']}")
        logging.info(f"[CM_CP] TP={self.cm_cp['TP']} TN={self.cm_cp['TN']} FP={self.cm_cp['FP']} FN={self.cm_cp['FN']}")
