...
class myController(object):
    def __init__(self):
        ...
        self.epoch_f = open(self.epoch_csv, "a", newline="")
        self.epoch_writer = csv.writer(self.epoch_f)

        self.det_f = open(self.out_csv, "a", newline="")        
        self.det_writer = csv.writer(self.det_f) 

        self.ml_f = open(self.ml_csv, "a", newline="")
        self.ml_writer = csv.writer(self.ml_f)
        self.epoch_has_attack_packet = False
        ...
        # --- performance knobs
        self.entropy_every = int(os.getenv('ENTROPY_EVERY', '10'))
    ...
    def query(self):
       ...
       # -------- DP SIZE (per epoch) --------
        ...
        for hh_id in range(0, self.heavysize):
            ...
            curr = max(self.counter[p4sw][0] for p4sw in self.counter) if self.counter else 0
            prev = self.prev_counter.get("global", curr)
            c = curr - prev
            # safety (can happen on restart)
            if c < 0:
                self.epoch_has_attack_packet = True

            self.prev_counter["global"] = curr 

            logging.info(f"[DP_DELTA] epoch={epoch_ts} curr={curr} prev={prev} delta_c={c}")

          
            ip = int_to_ip(self.hh_dip[hh_id]) 
            if ip == GT_VICTIM:
                self.epoch_has_attack_packet = True
            logging.info(f"[DEBUG] epoch={epoch_ts} c={c}")
            if c < DP_THRESHOLD :
                continue

            dp_pred = 1
            if not epoch_dp_triggered:
                epoch_dp_triggered = True
                dp_epoch_digests += 1
...
# --- epoch-level summary + confusion matrices (includes 'no alert')
        epoch_label = 1 if self.epoch_has_attack_packet else 0
        dp_alert = 1 if epoch_dp_triggered else 0
        ...
        logging.debug(f"[DP_EPOCH] epoch={epoch_ts} dp_size={dp_epoch_size} dp_alert={dp_alert}")
        ...
        # ---------- CSV writes (END OF EPOCH) ----------
        if det_rows_buffer:
            self.det_writer.writerows(det_rows_buffer)

        if ml_rows_buffer:
            self.ml_writer.writerows(ml_rows_buffer)
    ...
    def entropy(self):
        ...
        for p4sw in self.controllers:
            if len(p4sw) >= 4:      # skip divider
                continue
        try:
            self.counter[p4sw] = self.controllers[p4sw].register_read("counter")
        except Exception as e:
            logging.warning(f"[COUNTER_READ_FAIL] {p4sw}: {e}")
    ...
    def heavyhitter_only(self):
        ...
        #for hh_id in range(0, self.heavysize):
         #   if hh_id not in self.hh_dip.keys():
          #      continue
        for hh_id, dip in self.hh_dip.items():
            l1_hash = self.hh_hash[hh_id]
            leaf_values = []
            for p4sw in self.counter:
                if len(p4sw) == 3:   # s01, s02 (NOT divider)
                    leaf_values.append(sum(self.counter[p4sw][0]))
            curr = max(leaf_sum) if leaf_sums else 0
            
            logging.info(f"[COUNTER_SANITY] max={max(self.counter[p4sw])} sum={sum(self.counter[p4sw])}")
            if (c < 1000):
                continue
...
if __name__ == "__main__":
    ...
    try:
        while (time.time() - start_time) < TOTAL_TIME:
            ...
            if controller._epoch_counter % max(controller.entropy_every, 1) == 0:
                controller.entropy()
            t0 = time.time()
            controller.query()
