...
import pickle
DP_THRESHOLD = 1000
ATTACK_MODE = int(os.getenv("ATTACK_MODE", "0"))  # 0=benign, 1=attack
# ================= Ground Truth Config =================
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.cp_epoch_csv):
            with open(self.cp_epoch_csv, "w", newline="") as f:
                w = csv.writer(f)
                w.writerow(["epoch_ts","ip","cm_count","lemon_est","entropy","delta_lemon","dp_pred","cp_pred"])
...
# ================= CVFDT INTEGRATION START =================
            ...
            ml_rows_buffer.append([epoch_ts, ip, int(c), float(Est), float(entropy_val), float(delta_lemon), dp_pred, cp_pred])
