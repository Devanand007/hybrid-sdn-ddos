       ...
       # -------- DP SIZE (per epoch) --------
        ...
        for hh_id in range(0, self.heavysize):
           ...
           # dp_epoch_digests += 1
            ...
            logging.debug(f"[DELTA] ip={int_to_ip(self.hh_dip[hh_id])} delta_c={delta_c}")
            if delta_c < DP_THRESHOLD:
                continue
            dp_epoch_size += max(0, delta_c)
            ...
            if delta_c > 0:
                dp_epoch_size += delta_c


            if delta_c >= DP_THRESHOLD and not epoch_dp_triggered:
                dp_epoch_digests = 1
            ...
            logging.debug(f"[CMS_RAW] ip={ip} c={c} prev_c={prev_c}")
            if ip == GT_VICTIM:
